...
const OpponentTimer = () => {
  ...
  useEffect(() => {
    if (opponentTimer > 0 && opponentTimer <= 10) {
      Animated.loop(
        Animated.sequence([
  ...
  const getTimerColor = () => {
    if (opponentTimer === 0) return '#666666';
    if (opponentTimer <= 10) return '#FF4444';
    if (opponentTimer <= 20) return '#FFA500';
    return '#FF6B4A';
  };
  ...
  const isActive = opponentTimer > 0;
  const isUrgent = opponentTimer > 0 && opponentTimer <= 10;

  return (
    <View style={styles.container}>
      <View style={[styles.labelContainer, !isActive && styles.inactiveLabel]}>
        <Text style={styles.labelText}>⏱️ ADVERSAIRE</Text>
      </View>
